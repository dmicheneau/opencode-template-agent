...
export function detectProjectProfile(directory, _depth = 0) {
  ...
  if (existsSync(pkgPath)) {
    ...
    try {
      // Guard: skip symlinks and files > 5MB (malformed or unusual)
      const lstat = lstatSync(pkgPath);
      if (!lstat.isSymbolicLink() && lstat.size <= 5 * 1024 * 1024) {
  ...
  if (hasPyproject || hasRequirements) {
    ...
    try {
      ...
      // Guard: skip symlinks and files > 5MB
      const lstat = lstatSync(pyPath);
      if (!lstat.isSymbolicLink() && lstat.size <= 5 * 1024 * 1024) {
  ...
  if (existsSync(join(absDir, 'Gemfile'))) {
    languages.add('ruby');
    try {
      const gemPath = join(absDir, 'Gemfile');
      const lstat = lstatSync(gemPath);
  ...
  if (_depth === 0 && languages.size < 3) {
    ...
    for (const monoDir of MONOREPO_DIRS) {
      ...
      try {
        for (const entry of readdirSync(monoPath, { withFileTypes: true }).slice(0, 50)) {
...
export function analyzeQuery(prompt) {
  // Guard: strip control characters to prevent injection / pathological regex input
  let sanitized = prompt.replace(/[\x00-\x1f\x7f]/g, '');
  // Guard: truncate at 2000 chars to avoid pathological inputs
  const input = sanitized.length > 2000 ? sanitized.slice(0, 2000) : sanitized;
...
export function scoreAgents({ profile, query, installed, manifest }) {
  ...
  // ── 6. Final score clamp — ensure no score exceeds 1.0 after all boosts ──
  for (const entry of scores) {
    entry.score = Math.min(entry.score, 1.0);
  }
