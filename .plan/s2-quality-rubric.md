...
Rubric for evaluating agent definition files (~60-120 lines, YAML frontmatter + 7 required sections: Identity, Workflow, Decisions, Tools, Quality Gate, Anti-patterns, Collaboration). Each dimension is scored 1-5. An agent passes if the overall average is >= 3.5 and no single dimension falls below 2.
...
| **2** | Mostly generic with a few domain terms sprinkled in. Less than 30% of directives are role-unique. Could apply to several adjacent roles. |
...
| **4** | Most instructions are role-specific with precise vocabulary. A reader could narrow it to 2-3 possible roles. 70%+ of directives are role-unique. |
...
- Count domain-specific terms (framework names, API references, tool names) vs generic phrases ('be thorough', 'ensure quality'). High ratio of specific terms = high score. No pre-built keyword list needed — the scorer checks for generic anti-patterns rather than requiring role-specific lists.
...
| **2** | 2 decision points exist but are shallow (single IF without ELSE) or trivially obvious. |
| **3** | 3-5 explicit decision points. The Decisions section exists and contains at least 2 non-trivial branches. Some edge cases are covered. |
| **4** | 5-6 decision points with multi-branch logic. Covers happy path and some error cases. Most decisions reference concrete signals. |
| **5** | 7+ decision points spread across Workflow and Decisions sections. Covers happy path, error cases, and ambiguous situations. Decisions reference concrete signals (metric thresholds, file patterns, error codes). |
...
| **2** | Numbered steps exist but most lack actions or outputs. Steps are vague phase names ("Planning", "Execution") with minimal detail. |
...
| **4** | Most steps are verb-first with clear actions and outputs. 1-2 steps have implicit transitions but overall flow is clear. Step count is 4-10. |
...
| **2** | Permissions exist but have 3+ unused extras or miss significant capabilities the workflow requires. |
...
| **4** | Permissions closely match workflow needs with at most 1 minor discrepancy. Archetype is correctly identified. |
| **5** | Every permission maps to a specific action in the Workflow or Tools section. No unused permissions. No missing permissions. A justification comment or reference exists for non-obvious permissions. |

**Automated check hint:**
- Extract permissions from YAML frontmatter: `r"permission:\s*\n((?:\s+\w+:.+\n)*)"` (note: `permission` singular, nested dict).
...
| Score | Description |
|-------|-------------|
| **1** | Over 30% of lines are filler, boilerplate, or repeated information. File exceeds 200 lines or is under 40 lines. Contains paragraphs that could be replaced by a single directive. |
| **2** | 15-30% filler. File is 160-200 lines. Several paragraphs could be tightened significantly. |
| **3** | Minor filler present but most lines carry information. File is within 120-160 line range. Some sentences could be tightened but none are purely decorative. |
| **4** | Very little filler (<5%). File is 100-120 lines. Nearly every sentence carries actionable information. |
| **5** | Every line earns its place. No filler phrases. File is 60-120 lines and covers all 7 required sections with room to spare. Reads like a compressed checklist, not an essay. |

**Automated check hint:**
- Total line count: flag if > 120 or < 40.
...
The Tools section should name concrete tools (CLI commands, APIs, scripts, MCP servers) and explain *when* to reach for each one — not just list them.

| Score | Description |
|-------|-------------|
| **1** | No tools mentioned, or tools listed without context. Tools section is missing or contains only a bulleted list of names. |
| **2** | 1-2 tools mentioned with minimal context. No "when to use" conditions. Tools section exists but is superficial. |
| **3** | 2-4 tools referenced with brief usage context. At least one tool has a "when to use" condition. Tools section exists as a distinct section. |
| **4** | 3-4 tools with triggering conditions. Includes at least one "prefer X over Y" guidance. Some inline tool references in Workflow. |
...
- Check for tool mentions outside the Tools section header (inline integration).
...
| **2** | 1 domain-specific anti-pattern mentioned, or 2-3 that are too vague to be actionable. |
...
| **4** | 4 anti-patterns with concrete scenarios. Most include *why* the anti-pattern is dangerous. Covers common mistakes. |
...
| **2** | Mentions other agents exist but without specific handoff triggers or named targets. Vague awareness of multi-agent context. |
...
| **4** | 2-3 handoff scenarios with named agents. Includes clear trigger conditions. At least one "delegate to" flow. |
...
def score_agent(content: str) -> dict:
    ...
    # Extract permission keys (write, edit, bash, task, etc.)
    perm_keys = set(permissions.keys()) if isinstance(permissions, dict) else set()
    ...
    elif 4 <= step_count <= 10 and vf_ratio >= 0.6:
        scores["workflow_clarity"] = 4
    elif step_count >= 3:
        scores["workflow_clarity"] = 3
    elif step_count >= 2:
        scores["workflow_clarity"] = 2
    else:
        scores["workflow_clarity"] = 1

    # 4. Permission Alignment
    tool_refs_in_body = set(RE_TOOL_REF.findall(body))
    unused = perm_keys - tool_refs_in_body
    missing = tool_refs_in_body - perm_keys  # approximate
    if len(unused) == 0 and len(missing) <= 1:
        scores["permission_alignment"] = 5
    elif len(unused) == 0 and len(missing) <= 2:
        scores["permission_alignment"] = 4
    elif len(unused) <= 2:
    ...
    if filler_ratio <= 0.05 and 40 <= total_lines <= 120:
        scores["density"] = 5
    elif filler_ratio <= 0.05 and total_lines <= 140:
        scores["density"] = 4
    elif filler_ratio <= 0.15 and total_lines <= 160:
        scores["density"] = 3
    elif filler_ratio <= 0.30 and total_lines <= 200:
        scores["density"] = 2
    else:
        scores["density"] = 1
    ...
    elif len(unique_tools) >= 3 and conditional_refs >= 1:
        scores["tool_awareness"] = 4
    elif len(unique_tools) >= 2:
        scores["tool_awareness"] = 3
    elif len(unique_tools) >= 1:
        scores["tool_awareness"] = 2
    else:
    ...
    if ap_count >= 5:
        ...
        scores["antipattern_coverage"] = 4
    elif ap_count >= 2:
        scores["antipattern_coverage"] = 3
    elif ap_count >= 1:
        scores["antipattern_coverage"] = 2
    else:
        scores["antipattern_coverage"] = 1

    # 8. Collaboration Clarity
    handoff_matches = RE_HANDOFF.findall(body)
    body_mentions_agents = bool(re.search(r"`\w+-\w+`", body))
    if len(handoff_matches) >= 3:
        scores["collaboration_clarity"] = 5
    elif len(handoff_matches) >= 2:
        scores["collaboration_clarity"] = 4
    elif len(handoff_matches) >= 1:
        scores["collaboration_clarity"] = 3
    elif body_mentions_agents:
        scores["collaboration_clarity"] = 2
    else:
...
dimensions:
  ...
  collaboration_clarity:  4  # Delegates to `security-auditor` and `microservices-architect`, receives from `project-manager`
...
- Decisions cover: severity >= critical → block merge; test coverage < threshold → flag; unfamiliar language → delegate to specialist.
- Tools section names 4 tools with "use X when Y" conditions.
...
- Tools section says "Use appropriate tools as needed."
